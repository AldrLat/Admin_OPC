...
unit Vcl.CustomizeDlg;            //мой локальный
...
begin
  if Application.MessageBox('Сбросить все данные об использовании?',
                             PChar(ProgName_ShortStringVersion + ' ВНИМАНИЕ !!!'),
                             MB_YESNO + MB_ICONQUESTION) = IDYES  then
     if Assigned(FActionManager) then
       ActionManager.ResetUsageData;
//  if MessageDlg('Сбросить все данные об использовании?', mtConfirmation, [mbOK, mbCancel], 0) = mrOK then
//    if Assigned(FActionManager) then
//      ActionManager.ResetUsageData;
end;
